 ...
 *
 * Note: metric names in this module omit the grpc_server_ prefix because the
...
public class MonitoringServerInterceptorIntegrationTest {
    ...
    public void noHistogramIfDisabled() throws Throwable {
        ...
        createGrpcBlockingStub().sayHello(REQUEST);
        assertThat(
                PrometheusRegistryHelper.findRecordedMetric("handled_latency_seconds", prometheusRegistry)
                        .isPresent())
                .isFalse();
    }
    ...
    public void overridesHistogramBuckets() throws Throwable {
        ...
        assertThat(
                countSamples("handled_latency_seconds", "handled_latency_seconds_bucket"))
                .isEqualTo(expectedNum);

        FakeSample sample =
                getSample(findRecordedMetricOrThrow("handled_latency_seconds"),
                        "handled_latency_seconds_bucket");
    ...
    public void recordsHeadersAsLabels() throws Throwable {
        ...
        StreamRecorder<HelloResponse> streamRecorder = StreamRecorder.create();
        StreamObserver<HelloRequest> requestStream =
                createGrpcStub().withInterceptors(MetadataUtils.newAttachHeadersInterceptor(metadata)).sayHelloBidiStream(streamRecorder);
        requestStream.onNext(REQUEST);
        requestStream.onNext(REQUEST);
        requestStream.onCompleted();
