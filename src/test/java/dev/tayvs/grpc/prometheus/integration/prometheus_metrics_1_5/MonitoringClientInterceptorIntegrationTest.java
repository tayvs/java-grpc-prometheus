...
import dev.tayvs.grpc.prometheus.integration.prometheus_metrics_1_5.PrometheusRegistryHelper.FakeSample;
import dev.tayvs.grpc.prometheus.integration.prometheus_metrics_1_5.PrometheusRegistryHelper.FamilySamples;
import org.junit.After;
import org.junit.Before;
import org.junit.Test;

/**
 * Integration tests for the client-side monitoring pipeline. Uses the prometheus_metrics
 * (prometheus-metrics-core 1.x) implementation.
 *
 * <p>Note: the prometheus-metrics 1.x builder has no namespace/subsystem helpers, so the full
 * grpc_client_ prefix is baked into each metric name (e.g. grpc_client_started).
 */
public class MonitoringClientInterceptorIntegrationTest {
  ...
  public void unaryRpcMetrics() throws Throwable {
    ...
    FamilySamples handled = findRecordedMetricOrThrow("grpc_client_completed");
    assertThat(handled.samples).hasSize(2);
    FakeSample totalSample = getSample(handled, "grpc_client_completed_total");
  ...
  public void recordsHeadersAsLabels() throws Throwable {
    ...
    StreamObserver<HelloProto.HelloRequest> requestStream =
        createClientStub(ALL_METRICS.withLabelHeaders(Arrays.asList("header-1", "header-2")))
    ...
    FakeSample sample = getSample(findRecordedMetricOrThrow("grpc_client_completed"), "grpc_client_completed_total");
  ...
  public void noHistogramIfDisabled() throws Throwable {
    ...
    assertThat(
            PrometheusRegistryHelper.findRecordedMetric(
                    "grpc_client_completed_latency_seconds", prometheusRegistry)
                .isPresent())
        .isFalse();
  }
  ...
  public void addsHistogramIfEnabled() throws Throwable {
    createClientStub(ALL_METRICS)
        .sayHello(HelloProto.HelloRequest.getDefaultInstance(), responseRecorder);
    responseRecorder.awaitCompletion();
    FamilySamples latency = findRecordedMetricOrThrow("grpc_client_completed_latency_seconds");
  ...
  public void overridesHistogramBuckets() throws Throwable {
    ...
    assertThat(countSamples("grpc_client_completed_latency_seconds", "grpc_client_completed_latency_seconds_bucket"))
  ...
  private HelloServiceStub createClientStub(Configuration configuration) {
    return HelloServiceGrpc.newStub(
        InProcessChannelBuilder.forName(grpcServerName)
            .usePlaintext()
            .intercept(
                MonitoringClientInterceptor.create(new Provider(configuration, prometheusRegistry)))
            .build());
  }
